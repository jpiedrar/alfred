"""Searcharr Telegram command registration."""
...
from bot.commands.reset import notify_reset_complete, reset_command
from bot.commands.set_admin_chat import set_admin_chat_command
...
def register_commands(application, bot):
    ...
    # run_polling invokes this after the replacement process is initialized.
    application.post_init = notify_reset_complete
    ...
    logger.debug("Registering [/reset] as an authenticated-user restart command")
    application.add_handler(
        CommandHandler("reset", lambda update, context: reset_command(update, context, bot))
    )

    logger.debug("Registering [/setadminchat] as an admin-only configuration command")
    application.add_handler(
        CommandHandler(
            "setadminchat",
            lambda update, context: set_admin_chat_command(update, context, bot),
        )
    )
